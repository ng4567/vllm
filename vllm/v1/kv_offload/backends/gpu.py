...
from vllm.v1.kv_offload.mediums import DestGPULoadStoreSpec


class GPUBlockStatus(BlockStatus):
...
class GPUBackend(Backend):
    def __init__(self, block_size: int, num_blocks: int):
        super().__init__(block_size=block_size, medium=DestGPULoadStoreSpec.medium())
    ...
    def allocate_blocks(self, block_hashes: list[BlockHash]) -> list[BlockStatus]:
        ...
        for _ in range(num_fresh_blocks):
            blocks.append(GPUBlockStatus(self.num_allocated_blocks))
        ...
        for _ in range(num_reused_blocks):
            ...
            blocks.append(GPUBlockStatus(block_id))

        return blocks

    def free(self, block: BlockStatus):
        assert isinstance(block, GPUBlockStatus)
    ...
    def get_load_store_spec(
        ...
        return DestGPULoadStoreSpec([block.block_id for block in blocks])
