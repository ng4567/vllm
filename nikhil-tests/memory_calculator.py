...
class ModelConfigMemUsage:
    ...
    def __repr__(self):
        return (f"\nModelConfigMemUsage(model_name={self.huggingface_model_name}, "
                ...
                f"\nmodel_weight_memory={self.model_weight_memory:.2f} GB, "
                f"\ntotal_memory_usage={self.total_memory_usage:.2f} GB)")

def get_model_size(model_name: str, model_params_billion: float, avg_input_length: int, 
                   avg_output_length: int, max_num_sequences: int, huggingface_api_key: str = None,
                   verbose: bool = False) -> ModelConfigMemUsage:
  ...
  ConfigMemUsage.activation_memory = pytorch_activation_peak_memory_gb
  ConfigMemUsage.kv_cache_memory = kv_cache_memory_per_batch_gb
